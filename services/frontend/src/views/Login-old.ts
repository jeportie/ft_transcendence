...
export default class Login extends AbstractView {
    ...
    mount() {
        ...
        pwdWrapper.className = "relative w-full";

        // Move the input inside the wrapper
        userPwd.parentNode?.insertBefore(pwdWrapper, userPwd);
        pwdWrapper.appendChild(userPwd);
        ...
        icon.src = showIcon; // start with "show" icon
        icon.alt = "Show password";
        icon.className = "w-5 h-5 select-none";

        toggleBtn.appendChild(icon);
        pwdWrapper.appendChild(toggleBtn);
